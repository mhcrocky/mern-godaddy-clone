...
const Navbar = () => {
    ...
    const DropDownItem = ({ handleClick, children }: { handleClick: () => void, children: ReactNode }) => {
        ...
        return (
            <li>
                <span onClick={handleClick} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">
    ...
    return (
        <nav className="bg-white border-gray-200 dark:bg-gray-900">
            ...
            <div className="flex flex-wrap items-center justify-between px-8 p-4">
                <Link to="/" onClick={() => navigate('/')} className="lg:hidden flex items-center space-x-3 rtl:space-x-reverse pb-1 md:pr-20  pr-0">
                    <img src={Logo} className="h-8 md:pr-20 pr-0" alt="Logo" />
                </Link>
                <div className="flex items-center lg:order-2 space-x-1 lg:space-x-0 rtl:space-x-reverse min-w-[130px] items-center">
                    ...
                    <div className={`z-50 ${dropdownShow ? "" : "hidden"} my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 absolute top-[50px] xl:right-[80px] right-[130px]`}>
                    ...
                    <button onClick={() => toggleMenu(!menuShow)} type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg xl:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
                        <MenuToggleIcon />
                    </button>
                </div>
                <div className={`items-center justify-between ${menuShow ? "" : "hidden"} w-full lg:flex lg:w-auto lg:order-1`} id="navbar-language">
                    <Link to="/" onClick={() => navigate('/')} className="lg:block hidden flex items-center space-x-3 rtl:space-x-reverse pb-1">
                        <img src={Logo} className="h-8 pr-6" alt="Logo" />
                    </Link>
                    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 hidden xl:flex">
